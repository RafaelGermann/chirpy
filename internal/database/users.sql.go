...
const upgradeUserChirpRed = `-- name: UpgradeUserChirpRed :one
UPDATE users SET is_chirp_red = true, updated_at = NOW()
WHERE id = $1
RETURNING id, created_at, updated_at, email, hashed_password, is_chirp_red
`

func (q *Queries) UpgradeUserChirpRed(ctx context.Context, id uuid.UUID) (User, error) {
	row := q.db.QueryRowContext(ctx, upgradeUserChirpRed, id)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
